...
GNC::StreamingLoader::StreamingLoader(): DirectionCosines(GNC::GCS::IGinkgoMatrix4x4::New())
{
    ...
    //恢复加载私有Tags--私有Tag的Group，基本上为奇数。
	//如果该参数，被设置为false，会将跳过Group对应的Value长度的字节文件。
    m_IO->SetLoadPrivateTags(true);
...
//			char age[20];
	...
	catch (itk::ExceptionObject& ex)
	{
		...
		//GIL::DICOM::IDICOMManager* m = new GIL::DICOM::DICOMManager();
		...
		//GIL::DICOM::DICOMManager* iDicom = dynamic_cast<GIL::DICOM::DICOMManager*>(m);
		//if (iDicom != NULL) {
		//	delete iDicom;
		//}
		...
		//	for (int j = 0; j < 3; j++) {
		//		for (int i = 0; i < 3; i++) {
		//			m_DirectionCosines[(3 * j) + i] = ( (i == j)? 1.0 : 0.0 );
		//		}
		//	}
		//	DirectionCosines->Identity();
		//	RecomponerPipeline();
		...
		//}
		//else {
		//	//throw GNC::GCS::ControladorCargaException( ("Error reading the study: ") + ex.GetDescription(), "ControladorCarga/CargaMultidimensional");
		//}
	}
	catch (...)
	{
		...
		throw GNC::GCS::ControladorCargaException( ("Internal error reading the study, unknown format "), "ControladorCarga/CargaMultidimensional");
	}
}
...
void GNC::StreamingLoader::GetDirections(double cosines[9])
{
	for (int i = 0; i < 9; i++)
		cosines[i] = m_DirectionCosines[i];
}
...
void GNC::StreamingLoader::RecomponerPipeline()
{
	if (m_SignalFile) {
		...
		switch(m_IO->GetNumberOfComponents()) {
			case 1:
				{
					switch(m_IO->GetComponentType()) {
						...
						case ImageIO::UNKNOWNCOMPONENTTYPE:
						default:
							throw GNC::GCS::ControladorCargaException( ("Error reading the study: unsupported pixel format"), "ControladorCarga/CargaMultidimensiona(1 componente)");
					}
				}
				break;
			case 3:
				{
					switch(m_IO->GetComponentType()) {
						...
						case ImageIO::UNKNOWNCOMPONENTTYPE:
						default:
							throw GNC::GCS::ControladorCargaException( ("Error reading the study: unsupported pixel format"), "ControladorCarga/CargaMultidimensional(3 componentes)");
			...
			default:
				{
					throw GNC::GCS::ControladorCargaException( ("Error reading the study: unsupported pixel format"), "ControladorCarga/CargaMultidimensional");
				}
		}
	}
